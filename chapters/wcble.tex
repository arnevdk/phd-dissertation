...
Visual oddball \acp{bci} establish communication with paralyzed
...
Experiments show that latency jitter in \ac{erp}s is the result of the interacion of
...
in~\autoref{fig:smearing}. In \ac{erp} analysis, single-trial \ac{erp}s are usually
...
\ac{erp}s for jitter before averaging.\todo{reference}
...
\begin{enumerate*}[label={(\arabic*)}]
	...
	properties like peak amplitude and latency of \ac{erp}s across
	...
	other descriptors from a set of single-trial \ac{erp}s, like
  ...
  variability itself is determined as a correlate of working memory performance.
	...
	ratio of averaged \ac{erp}s by increasing the amplitude of the
  ...
  components which would otherwise obscured or blurred due to desynchronization.
	These deblurred templates can then be used for analysis of the \ac{erp}
...
\end{enumerate*}
\todo{Read, summarize and cite BCI	references from bibliography}
...
The most straight-forward method to estimate single-trial latency
peak-picking. Here, the latency of an \ac{erp} is determined as the
time-point of it's maximum (or minimum) amplitude with relaion to the stimulus
onset. While straight-forward, this method
does not perform well in low or negative SNR conditions, unless combined with
filtering. Filtering can suppress the noise contaminating EEG trials, lowering
the risk of picking a noise peak instead of a true peak of the \ac{erp}. Several filtering approaches have been developed to be used in
conjuction with peak-picking, in the time domain~\cite{Carlton1980, McCarthy1981,
	Karjalainen1999, Nishida1999,  Sparacino2002, Georgiadis2005,
	D’Avanzo2011} and in the time-frequency
domain~\cite{Quiroga2003, wang2007single,Hu2010,arico2014influence}. Filtering can
also be done in the spatial domain. The spatial filter can be constructed
using PCA~\cite{Ouyang2017}, ICA~\cite{townsend1999analyzing, jung2001analysis,
	Milne2011}, or as a spatial LCMV beamformer~\cite{TREDER2016279}.

A generally more preferred approach than
peak-picking is template matching. Woody's algorithm~\cite{woody1967characterization} is a simple and performant latency
estimation method that is still being used and is considered amongst the more
performant techniques~\cite{Ouyang2017}. It starts with the average \ac{erp} as a
rough estimation of the aligned template \ac{erp}, and iteratively refines this
template by determining at each step the time point of maximum
cross-correlation of this template with all single-trial \ac{erp}s and aligning them
...
\cite{souloumiac2013improved} uses XDAWN to construct spatial filter before
...
The abovementioned methods do not take into account the fact that  \ac{erp}s are multi-component signals. Multiple components can be time-locked to different events or neurophysiological proccess and hence are not necessarily
...
To categorize:\todo{categorize and analyze these references}
\begin{itemize}
...
\todo{check overlap with:


The most straightforward latency estimation method is \textit{peak picking}.
Peak picking determines the latency of an \ac{erp} as the time point of its maximum
...
e.g. the average target \ac{erp}, and iteratively enhances the SNR of this template by determining at
each iteration the time point of maximum cross-correlation of the template
with all single-trial \ac{erp}s, and aligning them based on these latencies to form
...
When multiple clusters of \ac{erp} components are not time-locked to
...
warping-based methods instead of explicit \ac{erp} component latency estimation can
also overcome the smearing problem.
Examples that have been applied to \ac{erp}s are Dynamic Time
...
\end{equation}
with $R = S-(s_2-s_1)$.
To leverage CBLE for \ac{erp} classification, the score vectors $\mathbf{y}$ can be
...
The \ac{erp} decoding method based on CBLE introduced by \cite{Mowla2017} does
...
\subsection{Classifier-based Latency Estimation with Woody
	Iterations}
