...
% Kronecker-structured LDA ====================================================
  ...
  published as~\textcite{VanDenKerchove2022}}.
	...
	%The study we report on focuses on the visual P300 oddball \ac{erp} in response to a
	%rare but attended visual stimulus.
	%The decoder detects whether this \ac{erp} is present to determine which stimulus
	%the user attended.
	%The P300 paradigm has been used extensively in \ac{bci} development and is easy to
	%set up~\cite{Farwell1988, Sellers2006, Barachant2014, Philip2020}.

  There are multiple state-of-the-art P300 classification methods, like
  ...
  $\hat{\mat{S}} \in \mathbb{R}^{C\times C}$ and $\hat{\mat{T}} \in \mathbb{R}^{S\times S}$ respectively correspond to estimators of the spatial and temporal covariance of the data.
	...
	\Cref{prop:inverse_kronecker} then leads to
  ...
  full, high dimensional Kronecker product $\hat{\mat{S}}^+\otimes
    \hat{\mat{T}}^+$.
	\Cref{fig:kronlda-covs}e and \cref{fig:covs}f respectively show examples of the
  ...
  \subsubsection{\Acl{stbf} classifier}
	...
	in
  \crefrange{eq:fpi_spatial}{eq:fpi-temporal-norm}
  ...
  \begin{figure}
    ...
    \caption[Clasifier accuracy in function of available training data.]{%
      Accuracy of the different classifiers for all 21 subjects relative to the
			...
			\cref{fig:accuracy_ap} contains results for all numbers of trials.
  ...
  \begin{table}[p]
	...
	First, because of \cref{prop:inverse_kronecker} and
	...
	The results shown in \cref{fig:training_time} also confirm that the
	...
	%	the central and parietal regions, where the P300 \ac{erp} component is present.
	...
	%	identity matrix and assuming identity covariance in \cref{eq:closed_form} yields weights
	...
	%	that state that, in addition to the P300, the early N1 and P2 \ac{erp} components
