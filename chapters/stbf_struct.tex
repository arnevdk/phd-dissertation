...
%	classification model, which can aid in analyzing and developing spatiotemporal beamformer-based classifiers.
  ...
  channels vEOG and hEOG, according to adaptive Z-score
  thresholding.\todo{details? threshold for rejection?}
  ...
  This effect is present for 1-, 2- and 5-trial evaluation\todo{qualitative}.
	This advantage decreases (the $p$-value increases) when
  adding more training blocks\todo{qualitative}.
	Both \textsc{stbf-struct} and \textsc{stbf-shrunk} perform significantly better
  than \textsc{stbf-emp} for all evaluated settings\todo{qualitative}.
  ...
  training block\todo{qualitative}.
	\textsc{stbf-shrunk} does not outperform \textsc{xdawn+rg} when training data
  is low but gains a significant advantage	when using more training
  data\todo{qualitative}.
	...
	\end{figure}
  \todo{the legend does not correspond to the text (method names are
  different)}



	\cref{fig:stbf-struct/training-time} shows that the training time of
	\textsc{stbf-struct} has a lower median training time than
  \textsc{stbf-shrunk}, \textsc{stbf-emp} and \textsc{xdawn+rg} when training
  on all available data and using 32 channels\todo{qualitative}.
  ...
  training data is small\todo{qualitative}.
	...
	covariance.
  The increase in accuracy for small training set sizes\todo{qualitative} can also be explained by the smaller number of parameters necessary to estimate the inverse covariance (see \cref{sec:stbf-struct/discussion/param-complex}), increasing the stability of matrix inversions.
  ...
  better accuracies\todo{qualitative}.
  ...
  training data increases\todo{qualitative}.
	...
	calculations.
	It can be observed that the training time of \textsc{stbf-struct} stays low
  compared to \textsc{stbf-emp} and \textsc{stbf-shrunk} when dimensionality
  increases\todo{qualitative}.
  ...
  substantially less memory usage\todo{qualitative}.
