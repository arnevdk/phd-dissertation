...
\todo{It's always tricky in the related work section of an ML/Mathematical
paper, but unless your reviewers are absolute experts of this specific field,
...
\end{equation}
as visualized in \autoref{fig:HODA-backward}.
\begin{figure}[t]
	\centering
...
The full HODA procedure is summarized in \autoref{alg:HODA}.
\begin{algorithm}
...
The HODA projection in \autoref{eq:HODA-backward} is an example
of a backward model.
A straightforward and computationally efficient candidate for a corresponding
forward model, visualized in \autoref{fig:HODA-forward}, is given by:
\begin{equation}
...
\begin{equation}
  ...
  -\hat{\ten{X}}(n)\right)^2
  = \arg\min_{\{\mat{A}\}}\sum_n^N\left(\ten{X}(n) - \ten{G}(n)\mmprs{\mat{A}}\right)^2
\end{equation}
This least squares tensor approximation problem can be solved efficiently using the
alternating least squares (ALS) algorithm~\cite{Bentbib2022}, iteratively fixing all but one of the activation patterns such that:
\begin{equation}
...
\autoref{eq:HODA-forward} should still contain some discriminative
...
$\ten{\mathbfcal{E}}$.
\autoref{fig:BTTDA} further illustrates the BTTDA model.
\begin{figure*}[t]
	\centering
...
extraction.
If $B$ in \autoref{eq:BTTDA-forward} is set to one, BTTDA is equivalent to
HODA; if at each term $b$ the rank of the core tensor
$(r_1^{(b)}=r_2^{(b)}=\ldots=r_k^{(b)}=1)$, a PARAFAC structure is assumed.

Since BTTDA is specified above as a forward model, a backward procedure
...
computed through a deflation scheme summarized in \autoref{alg:BTTDA}.
\begin{algorithm}
...
projection in \autoref{eq:HODA-backward} from the residual error term of the previous
...
\autoref{alg:model-selection} proposes a heuristic model selection
...
demand.
Details about these datasets are found in \autoref{tab:moabb}.
...
greedy model selection algorithm in \autoref{alg:model-selection}.
...
by \autoref{tab:moabb} and band-pass filtered between 1Hz
and 24Hz.
The signals were cut into epochs starting from stimulus onset with a
dataset-specific length given by \autoref{tab:moabb}.
...
\autoref{tab:erp-score} lists the cross-validated ROC-AUC for all evaluated
decoders.
\begin{table}[t]
	\footnotesize
	\centering
...
The MI datasets listed in \autoref{tab:moabb} were first band-pass filtered
between 8 and 30Hz and cut into
epochs with time windows as specified by \autoref{tab:moabb}.
...
\autoref{tab:mi-score} lists the cross-validated classification scores for
...
of the number of blocks increases, shown in \autoref{fig:blocks}.
...
The results in \autoref{tab:erp-score} and \autoref{tab:mi-score} show that
...
\autoref{fig:blocks} shows that there is added value in finding extra blocks
...
Tucker structure, as illustrated by \autoref{fig:bttda/sparse}.
\begin{figure}
...
\autoref{fig:forward} shows the activation patterns
...
information in the class labels. \todo{spellcheck from here}
