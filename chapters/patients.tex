...
oddball \ac{bci}s (see \cref{sec:gaze-independence} for an overview), since users
...
It would be of interest to verify if such results can be replicated with other
...
using a suited decoding strategy.
That accounts for latency jitter in covert \ac{vsa} responses.
...
Of these, one participant was diagnosed with bulbar-onset \ac{als}.
...
whose life span has been extended with life support and assistive technology to
...
transformations of gaze positions to the stimulus space, they were assumed to be overtly performing the task.
