...
Christopher Paolini}
\noindent\emph{\Cref{sec:wcble/literature/contrib}, \cref{sec:wcble/methods} (including
...
For instance, \textcite{Zisk2021} showed that \ac{als} results in significantly
increased \ac{erp} jitter\todo{qualitative}.
...
This difference is particularly large\todo{qualitative} when $\sigma$ is higher ($\sigma=0.2$
...
and \ac{wcble} performed at chance leveltodo{qualitative}.
When considering the accuracy of \ac{wcble}, we see that it shows more of a
plateau as \ac{snr} increases\todo{qualitative}.
...
potential jitter and lower \ac{snr} than \ac{cble}\todo{qualitative}.
...
At a certain noise threshold\todo{qualitative}, both methods begin to fail, but \ac{wcble}
delivers higher accuracy at intermediate \ac{snr} levels than
\ac{cble}\todo{qualitative}, likely due to the iterative alignment process.
