...
Christopher Paolini}}
\publicationnote{\Cref{sec:wcble/literature/contrib},
\crefrange{sec:wcble/methods/cble}{sec:wcble/methods/wcble} (including
\cref{fig:diagram-train,fig:diagram-eval}), and the third-to-last paragraph of
\cref{sec:wcble/conclusion} were published as part of \textcite{VanDenKerchove2024}.}
...
\textcite{Souloumiac2013} use the XDAWN technique to construct a spatial filter before
...
\Pr(\mat{X},\theta,l,s) = \frac{1}{R}\Pr(\mat{X}[:,s:s+(s_2-s_1)],\theta,l)
  \quad \forall s\in 1,\ldots,R
	\label{eq:prob}
\end{equation}
The latency features assuming the epoch belongs to a given class given by
...
\fullpagefig[caption.fullwidth, odd]{%
  \includegraphics[width=\textwidth]{figures/wcble/figure1.pdf}
}{%
  \caption[Schematic representation of the \acs{wcble} training phase.]{%
    The \acf{wcble} training phase.
    (1) The first-stage classifier is trained on a set of
    epochs.
    (2) It is then applied to time-shifted copies of the epochs, yielding scores
    and class probabilities over time.
    (3) The medians of the probability distributions are assumed as the class
    latencies.
    (4) The epochs are aligned by shifting in time based on their corresponding class latencies by
    shifting in time.
    (5) The spatiotemporal classifier is then retrained on the aligned epochs
    for the next iteration.
    (6) On convergence, the scores and latencies obtained
...
\fullpagefig{%
  ...
  \caption[Schematic representation of the \acs{wcble} test phase.]{%
    The \acf{wcble} test phase. (1) The
	  first-stage classifier obtained from the training phase is
	  applied to time-shifted copies of the epochs, yielding scores and class probabilities
...
\fullpagefig[anchor.north]{%
  \hspace{1cm}\inputpgf{figures/wcble}{simulated-sine.pgf}
}{%
  \caption[Simulated target and non-target evoked data.]{%
    ...
    deviation).
    Targets contain a sine wave pulse, jittered by $\sigma$. 32 \ac{eeg}
    channels with varying levels of noise were simulated using a forward model, channel C3 is
    plotted here.
  }
...
\Cref{fig:wcble/results/accuracy} shows that the classification accuracy of all
