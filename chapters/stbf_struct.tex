...
%	classification model, which can aid in analyzing and developing spatiotemporal beamformer-based classifiers.
  ...
  \crefrange{tab:stbf-struct/accuracy/stbf-struct}{tab:stbf-struct/accuracy/xdawn-rg}
...
\begin{subappendices}
  ...
  \begin{table}[!h]
      ...
      \caption[Accuracies for \textsc{stbf-struct}.]{Accuracies (\%) for \textsc{stbf-struct}.}
      \label{tab:stbf-struct/accuracies/stbf-struct}
  \end{table}
  \begin{table}[!h]
	    \input{chapters/stbf_struct-acc-stbf_loocv.tex}
      \caption[Accuracies for \textsc{stbf-struct}.]{Accuracies (\%) for
      \textsc{stbf-shrunk}.}
