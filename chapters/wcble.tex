...
stimuli (see Figure~\ref{fig:bci/loop}).
These \acp{erp} consist of multiple \ac{erp}-components, peaks and troughs in
\ac{erp} waveform (see Figure~\ref{fig:bci/erp}.
...
\subsection{Neural origins of \ac{erp} latency jitter}
...
\subsection{Implications for \ac{erp} analysis}
...
considered amongst the more performant techniques for
...
\Ac{cble}, summarized in \autoref{sec:wcble/app/cble-alg}, works by training a
...
The W\ac{cble} algorithm is presented in \autoref{sec:wcble/app/wcble-alg}.
...
In \autoref{sec:covert-align}, the proposed algorithm will be applied to real
\ac{eeg} data.
...
\fullpagefig{%
  ...
  Simulated target (bottom half of epochs) and non-target (top half of epoch) evoked data
  and their average (red: target, yellow: noon-target, shaded are: standard
  deviation).
...
\begin{figure}
  ...
  target data, measured as pearson correlation coefficient $\rho$ between
...
\autoref{fig:wcble/results/accuracy} shows that the classification accuracy of both \ac{tlda} and \ac{cble} decreases with
