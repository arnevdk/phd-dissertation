...
Pink spatial and  temporal noise are then added.
Temporal noise is generated by passing gaussian noise through an infinite
impulse response filter with transfer function
$\frac{B(z)}{A(z)} = \frac{1}{1 -1z^{-1}+0.15z^{-2}}$.
For pink spatial noise, a noise covariance matrix is constructed from the
cosine distances between electrode positions and scaling the resulting spatial
frequencies by $1/f$.
