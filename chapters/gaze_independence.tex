...
\ac{tbi}).
Many individuals in these groups unfortunately also suffer
to some extent from eye motor impairment, requiring \acp{bci} adapted to their condition.
...
This group is much larger than the first.''~\cite{Wolpaw2008}}}
...
In clinical reality, every patient has different symptoms, which lead to their own
...
Instead of categorizing patients, we will refer to those who might benefit from
...
stimulation do not rely on gaze redirection but also result in lower information transfer
...
and temporally organized interfaces, like \ac{rsvp}, where targets or small sets of targets are shown consecutively.
...
Section~\ref{sec:gaze-independence/sota} showed that visual \ac{bci} paradigms are a
...
described in \autoref{sec:stbf-struct}.
...
gaze-independence.
The proposed \ac{bttda} method will be discussed in \autoref{sec:bttda}.
...
and evaluated on simulated data in \autoref{sec:wcble}.
...
a patient study with gaze-impaired individuals to evaluate the impact of eye motor deficits
on \ac{bci} performance.
This study focused on individuals with \ac{sspgi}, recruited from neurorehabilitation centers
...
The results of this study are presented in \autoref{sec:patients}.
